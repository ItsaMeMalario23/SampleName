...
output main(uint id : SV_VertexID)
{
    ...
    float2 tmp = float2(((vertexpos[vert].x * data[idx].scale) + data[idx].position.x - 1.0f + ds.x) * lscale, ((vertexpos[vert].y * data[idx].scale * 1.7777777778f) + data[idx].position.y - 1.0f + ds.y) * lscale);
