...
// MSAA
static SDL_GPUGraphicsPipeline* msaapipelines[4];
static SDL_GPUTexture* msaaextures[4];
static SDL_GPUTexture* resolvetexture;
static SDL_GPUTextureFormat rtfmt;
...
// hitbox buffer
static hitbox_t hitboxes[HITBOX_BUF_SIZE];
static u32 numboxes;
...
static void initStdPipeline(SDL_GPUDevice* restrict dev, SDL_GPUShader* restrict vert, SDL_GPUShader* restrict frag)
{
    if (!vert || !frag) {
...
static void initMSAAPipelines(context_t* restrict con, SDL_GPUShader* restrict vert, SDL_GPUShader* restrict frag)
{
    ...
    for (u32 i = 0; i < 4; i++) {
        SDL_GPUSampleCount scount = (SDL_GPUSampleCount) i;

        if (!SDL_GPUTextureSupportsSampleCount(con->dev, rtfmt, scount)) {
            SDL_Log("Sample count %d not supported", (1 << renderSamples));
            continue;
        }

        info.multisample_state.sample_count = scount;
        
        msaapipelines[numsamples] = SDL_CreateGPUGraphicsPipeline(con->dev, &info);

        if (!msaapipelines[numsamples]) {
            SDL_Log("Failed to create MSAA pipeline");
            return;
        }
        ...
        if (scount == SDL_GPU_SAMPLECOUNT_1)
            texinfo.usage |= SDL_GPU_TEXTUREUSAGE_SAMPLER;

        msaaextures[numsamples] = SDL_CreateGPUTexture(con->dev, &texinfo);

        if (!msaaextures[numsamples]) {
            SDL_Log("Failed to create MSAA texture");
            SDL_ReleaseGPUGraphicsPipeline(con->dev, msaapipelines[numsamples]);
        }
...
static void initHitboxPipeline(SDL_GPUDevice* restrict dev, SDL_GPUShader* restrict vert, SDL_GPUShader* restrict frag)
{
    if (!vert || !frag) {
    ...
    vtxbuf = SDL_CreateGPUBuffer(
        dev,
        &(SDL_GPUBufferCreateInfo) {
            .usage = SDL_GPU_BUFFERUSAGE_VERTEX,
            .size = sizeof(vec2f_t) * 6 * HITBOX_BUF_SIZE
        }
    );
}

static void initTexture(context_t* restrict con, const char* restrict filename)
{
    ...
    SDL_GPUTransferBuffer* textransferbuf = SDL_CreateGPUTransferBuffer(
        con->dev,
...
void renderInitPipelines(context_t* restrict con)
{
    ...
    rtfmt = SDL_GetGPUSwapchainTextureFormat(con->dev, con->window);
...
void renderDraw(context_t* restrict con)
{
    ...
    if (numchars && swapchain) {
        ...
        SDL_GPURenderPass* renderpass = SDL_BeginGPURenderPass(
            ...
            &(SDL_GPUColorTargetInfo) {
                ...
                .clear_color = (SDL_FColor) {CLEAR_COLOR_R, CLEAR_COLOR_G, CLEAR_COLOR_B, CLEAR_COLOR_A}
            },
            1,
            NULL
        );
    ...
    if (numboxes && swapchain) {
        ...
        vec2f_t* data = SDL_MapGPUTransferBuffer(con->dev, transfbuf, false);

        u32 k = 0;
        for (hitbox_t* i = hitboxes; i < hitboxes + HITBOX_BUF_SIZE; i++) {
            if (!i->object)
                continue;
...
void renderDrawMSAA(context_t* restrict con)
{
    SDL_GPUCommandBuffer* cmdbuf = SDL_AcquireGPUCommandBuffer(con->dev);
    ...
    if (!SDL_WaitAndAcquireGPUSwapchainTexture(cmdbuf, con->window, &swapchain, &w, &h)) {
        SDL_Log("Failed to acquire swapchain texture: %s", SDL_GetError());
        return;
    }
    ...
    if (swapchain) {
        ...
        SDL_GPUColorTargetInfo info = {
            .texture = msaaextures[renderSamples],
            .clear_color = (SDL_FColor) {CLEAR_COLOR_R, CLEAR_COLOR_G, CLEAR_COLOR_B, CLEAR_COLOR_A},
            .load_op = SDL_GPU_LOADOP_CLEAR
        };

        if (renderSamples == SDL_GPU_SAMPLECOUNT_1) {
            info.store_op = SDL_GPU_STOREOP_STORE;
        } else {
            info.store_op = SDL_GPU_STOREOP_RESOLVE;
            info.resolve_texture = resolvetexture;
        }

        SDL_GPURenderPass* renderpass = SDL_BeginGPURenderPass(cmdbuf, &info, 1, NULL);

        SDL_BindGPUGraphicsPipeline(renderpass, msaapipelines[renderSamples]);
        SDL_BindGPUVertexStorageBuffers(renderpass, 0, &fragdatabuf, 1);
        SDL_BindGPUFragmentSamplers(renderpass, 0, &(SDL_GPUTextureSamplerBinding) { .texture = texture, .sampler = sampler }, 1);
        ...
        SDL_GPUTexture* blitsrc = (info.resolve_texture) ? info.resolve_texture : info.texture;

        SDL_BlitGPUTexture(
            cmdbuf,
            &(SDL_GPUBlitInfo) {
                .source.texture = blitsrc,
                .source.x = 0,
                .source.w = 1920,
                .source.h = 1080,
...
void renderCleanupPipelines(context_t* restrict con)
{
    ...
    for (u32 i = 0; i < 4; i++) {
        SDL_ReleaseGPUGraphicsPipeline(con->dev, msaapipelines[i]);
        SDL_ReleaseGPUTexture(con->dev, msaaextures[i]);
    }

    SDL_ReleaseGPUTexture(con->dev, resolvetexture);
...
renderer_t r_ascii2D = {RENDERTYPE_2D, renderInitPipelines, renderDraw, renderCleanupPipelines};
...
{
    if (!numboxes) {
        SDL_Log("Failed to remove hitbox, not hitboxes in buf");
        return;
    }
    ...
    SDL_Log("Failed to remove hitbox, not hitbox for object in buf");
}
